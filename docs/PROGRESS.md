...
- Implemented Logistics Auth and its protected Logistics React SPA at `src/logistics`. Added the fifth `logistics` user role, UUID-backed personal profile, one-account/one-organization/one-operational-hub foundation, transactional pending registration with private image evidence, role-scoped password recovery, Sanctum session login/session restoration/logout, Admin registration-review integration, and stable active-status enforcement. The new `/dashboard` returns only the authenticated hub scaffold and safe freshness metadata; parcel, shipment, subscription, and Courier operations remain deliberately deferred until their contracts exist. Added local/production Logistics SPA CORS and stateful-Sanctum origins, plus Admin registration queue/detail/dashboard support for Logistics applications. Focused Logistics API tests pass (3 tests, 18 assertions), and Logistics/Admin TypeScript, lint, and production builds pass. PostgreSQL migration verification remains blocked by the local Laravel-to-PostgreSQL connection failure despite the Docker container running.
- Added an opt-in `InitialLogisticsSeeder` and documented `INITIAL_LOGISTICS_*` bootstrap variables. When credentials are provided, it creates one active Logistics account with its profile, organization, and sole operational hub; repeated seeding does not overwrite existing account data. Focused bootstrap and Logistics Auth tests pass (8 tests, 60 assertions).
- Rechecked the local PostgreSQL and Azure Blob connections outside the workspace network sandbox: Laravel authenticates to both services, the PostgreSQL migration preview is valid, and Azure Blob completed a read-only lookup. No database migration or Blob write was performed during this diagnostic.
- Implemented the mobile-only Courier authentication API: active-Logistics discovery, multipart Courier registration with private evidence, one derived Logistics organization/hub affiliation, pending/approved/rejected affiliation gate, Logistics-only approval API, role/status/affiliation middleware, Sanctum Courier baseline tokens, `me`, current-token logout, throttling, and generic recovery response. No Courier web UI was created. The focused registration test currently terminates the PHP runner without a diagnostic, so end-to-end Courier verification remains pending.
...
- Added an idempotent `CourierSeeder` and wired it into the main database seeder. It creates one configured Courier from `INITIAL_COURIER_*` values plus 20 deterministic generic Courier accounts using the same `INITIAL_COURIER_PASSWORD`, each with a profile, address, vehicle, and approved affiliation to the active seeded Logistics organization. Documented the Courier bootstrap and generic-account controls in `src/api/.env.example`; focused seeder coverage passes (1 test, 21 assertions).

- Aligned the shared order/logistics documentation with the settled first-mile and final-mile flow. Added explicit Shipment/Delivery Task vocabulary (`picked_up_from_seller`, `received_at_hub`, `sorted_at_hub`, `dispatched_from_hub`, `delivery_assigned`, `picked_up_from_hub`), preserved the current high-level `OrderStatus` contract, and synchronized the requirements, workspace, schema, and affected feature specs. No application behavior changed.
- Revised `docs/domains/Logistics.md` with the one-organization/one-hub MVP boundary, explicit first-mile/final-mile workflow, canonical lower-case status vocabulary, operational invariants, and deferred shipment/task schema boundaries. No application behavior changed.
- Revised `docs/domains/Courier.md` with the external mobile-only boundary, Logistics approval and sole-hub affiliation rules, explicit first-mile/final-mile Courier transitions, current/deferred data boundaries, and secure operational feature contracts. No application behavior changed.
- Clarified in `docs/domains/Courier.md` that first-mile and final-mile assignments are independent: Logistics may assign the same or a different eligible Courier, and first-mile completion never automatically grants final-mile work. No application behavior changed.
...
- Implemented Seller order read/detail, notification-read, and COD acceptance APIs with strict Shop scoping, UUID idempotency, locked `placed → seller_processing` transitions, immutable order snapshots, reservation preconditions, and an additive acceptance ledger. Checkout now sends one safe, deterministic Seller actionable-order notification after commit; acceptance never changes payment, inventory, courier, or waybill state. A generated Logistics waybill remains unavailable until its downstream domain exists. Focused Seller/Checkout coverage passes 9 tests/92 assertions, PHP formatting and diff checks pass, and PostgreSQL migration SQL validates.

- Implemented the Seller Accept Order frontend with a responsive light/dark inbox, URL-backed status/read filters and pagination, immutable order details/history, notification-read separation, accessible acceptance confirmation, persistent retry keys, conflict refresh, and visible-page polling. Connected Orders to the sidebar, notification shortcut, Dashboard queue, and a preparation review route with Inventory navigation; package submission/readiness and Logistics waybills remain explicitly unavailable. Fixed PostgreSQL notification UUID comparison and added validated filtering for all canonical order states. Five frontend action tests and 10 Seller/Checkout API tests (106 assertions) pass, along with Seller lint, TypeScript, production build, PHP formatting, and read-only PostgreSQL filter checks. Browser visual/interaction testing remains unverified because no browser test runtime is installed.

- Reorganized Seller Orders into sidebar subsections for Monitoring, Approval, and Pickup. Monitoring now shows Shop-scoped status cards with status/amount/activity sorting; Approval supports explicit approve or reject decisions; Pickup lets Sellers group up to 50 processing Orders into one pending Logistics request, moves them to `ready_for_pickup`, and safely notifies active Logistics accounts after commit while assignment and pickup date remain pending. Renamed the public/UI acceptance contract to Order Approval, added additive rejection/pickup ledgers, idempotent actions, reservation release on rejection, and pickup grouping. Fixed valid checkout Orders being rejected during approval by checking the authoritative Order-linked Inventory reservation instead of the checkout-batch reference. Six frontend action tests and 12 Seller/Checkout API tests (130 assertions) pass, together with Seller lint, TypeScript, production build, PHP formatting, and PostgreSQL migration SQL validation.
...
- Added implementation-ready cross-role specifications for Seller-selected Logistics pickup requests, Geoapify-assisted nearest-provider recommendations, a Logistics Pickups dashboard, 30-Order Courier schedules, and idempotent Seller/Courier schedule notifications/reminders. Added the shared Seller-created A6 PDF waybill contract with opaque QR resolution, packing/printing guidance, role-scoped Seller/Logistics access, Courier task authorization, and researched free/open-source PDF/QR dependencies. Reconciled the directly conflicting requirements, workflows, schema notes, domain contexts, and Seller/Customer/Logistics/Courier feature specifications; no application behavior or packages changed.

## 2026-09-08

- Implemented the Seller-to-Logistics pickup and shared-waybill API slice. Sellers receive PSGC-first, optional Geoapify road-distance-ranked providers and atomically commit one immutable selected organization, readiness transition, and A6 QR waybill per Order. Selected Logistics tenants receive private pickup queue/detail, waybill, 1–30 Order schedule, revision, and cancellation endpoints with approved affiliated-Courier checks, UTC windows, overlap/idempotency controls, append-only history, and durable one-hour reminders. Couriers receive mobile-only first-mile task list/accept and assignment-authorized opaque QR resolution endpoints; scheduling, document access, and scanning do not mutate custody, Inventory, payment, or Order status. Added locked Dompdf/BaconQrCode dependencies, restored the Geoapify location policy, and added cross-role regression coverage. Focused suites pass on SQLite (17 tests/181 assertions) and PostgreSQL 18.3 (10 tests/143 assertions); Composer reports no locked dependency advisories. The full SQLite API suite passes 229/231 tests, with two pre-existing date-frozen Recently Viewed tests failing because their September 3 clock makes newly created September 8 products future-dated.

## 2026-09-09

- Implemented the Seller-to-Logistics pickup frontend across the Seller and Logistics dashboards. Sellers can select ranked Logistics providers, submit prepared Orders, follow packing guidance, and preview/download/print immutable waybills; Logistics now has a responsive filtered Pickups queue, tenant-scoped detail view, 30-Order selection, approved-Courier scheduling, revision/cancellation flows, and waybill actions with explicit Asia/Manila times. Added the tenant-scoped eligible pickup-Courier read endpoint and redesigned the printer-safe A6 waybill with clearer delivery, routing, COD, item-count, reference, and QR hierarchy. Focused Logistics API tests pass with 4 tests/65 assertions; Seller order-action tests, Logistics/Seller lint, TypeScript, and production builds pass.

- Fixed Courier bootstrap affiliation so the configured initial Courier and generic Courier accounts are always approved employees of the configured initial Logistics organization and its sole hub. The seeder now repairs a stale/wrong affiliation on rerun, never falls back to another active organization, and has regression coverage for both cases.

- Fixed Seller Prepare Orders provider loading for the bootstrap Seller by seeding its required pickup address and documenting configurable address defaults. Removed focus-driven order refetches, reduced visible polling to once per minute while retaining explicit and post-mutation refreshes, and deduplicated short-lived Logistics-option reads so ordinary clicks and React development remounts do not generate repeated API traffic.

- Rebuilt the A6 waybill PDF as one bounded, collapsed table grid so section rules and the shared center divider connect consistently and header, routing, COD, and QR content align predictably. Removed the overflowing full-height/absolute-footer combination, verified long representative addresses visually, and added regression coverage requiring exactly one PDF page for a single waybill.

- Fixed Logistics pickup scheduling so the post-create detail refresh can load the assigned Courier, and overlapping Courier windows now keep the schedule dialog open with the API conflict message. Added Seller-scoped notification list/detail/read APIs, a header bell with recent notifications, dedicated notification pages, pickup assignment date/time details, and persistent PHT schedule summaries on Seller Pickup orders. Focused Logistics coverage passes with 5 tests/84 assertions; Seller and Logistics lint/build plus 7 Seller action tests pass.

- Restricted Customer Order notifications to important decisions and outcomes: Seller approval, cancellation/rejection, out-for-delivery/delivery completion, and delivery/return issues. Routine placement, preparation, first-mile readiness, assignment, pickup, and in-transit movements remain in Order tracking but no longer generate or appear as Customer notifications, including legacy records. Seller pickup-schedule notifications now provide meaningful event-specific titles and PHT schedule summaries for both new and older records that lack display text. The combined Customer Order, Checkout, Seller Order, and Logistics pickup suites pass with 24 tests/321 assertions.
