...
- Planned hub pin flow: complete PSGC/manual operational address → choose **Pin hub location** → intentional Geoapify lookup → adjust Leaflet pin to the actual hub → explicitly confirm → server validates/saves latitude and longitude together on the sole hub Address. During provider failure, preserve manual registration and display unpinned; Account Settings later adds/corrects the same-premises pin. No Mapbox or new approval/dispatch gate is introduced. Pin corrections preserve committed waybill/address/manifest history and invalidate only future derived route caches; relocation remains separately controlled.
