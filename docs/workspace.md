...
- Current registration creates one Vehicle, but database-wide one-vehicle enforcement still needs additive uniqueness on `vehicles.courier_profile_id` after safe duplicate/missing-row reconciliation. Do not reseed, delete duplicates automatically, or present this target constraint as deployed.
- Approved target self-service flow: active Courier edits type/plate/optional make/model or replaces OR or CR → server validates/locks the sole Vehicle and commits changed fields/document pointer plus notification intent → associated Logistics receives an informational alert, not a reapproval request. Omission preserves the other image; failure preserves existing data. Initial approval/evidence and task snapshots remain unchanged. Separate-document endpoints and registration rollout are not implemented; their owner is Logistics Vehicle Fleet Management.
...
As defined in `docs/requirements.md`, Admin owns platform-wide voucher management applicable at Customer checkout.
