...
This is the target cross-role flow. The currently implemented foundation stops at Seller pickup requests, the immutable shared waybill, pickup schedules, and first-mile assignment/acceptance, explicit Seller pickup confirmation, inventory fulfillment, and Courier route manifests. Logistics-validated physical scan records, hub processing transitions, final-mile tasks, and proof-of-delivery require the approved shared Shipment/Parcel/Delivery Task contract and additive migrations before their endpoints or UI are enabled.
