...
`waybills` has one UUID row per Order through unique `order_id`, with unique non-sequential human reference and keyed QR-payload hash. The one-page A6 PDF renders both the opaque QR and a Code 128 form of the human reference locally. It retains the pickup request, Shop, selected organization, sole hub, status, template/schema versions, and immutable content checksum. `waybill_snapshots` stores the server-owned printable payload one-to-one; `waybill_access_events` appends authorized view/download/bulk-download/resolve actions without claiming physical printing or custody.
