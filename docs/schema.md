...
- **Implementation gap:** the inspected `ConfirmFirstMilePickup` updates the task and bridges fulfillment but does not complete the schedule. `PickupScheduleService` already limits conflicts and revise/cancel eligibility to `scheduled`; reminder dispatch still needs completion-aware guards. These requirements are not a claim of deployed completion/backfill behavior.
- Existing zero-remaining `scheduled` rows require an in-place, bounded, idempotent reconciliation/backfill, not `migrate:fresh` or reseeding. Lock and revalidate schedule status and complete membership; only schedules with linked tasks all picked up qualify. Complete and suppress pending reminders atomically; report empty, missing, cancelled, or inconsistent task sets without inventing custody. Preserve existing references/history and do not rerun pickup or stock effects. No reconciliation is executed by this documentation change.
