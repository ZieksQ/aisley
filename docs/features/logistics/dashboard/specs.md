...
- **Parcel search behavior:** `/operations` requests ten online queue rows at a time and uses the existing server pagination contract. It stores only safe queue/detail projections in a browser cache keyed by the authenticated organization and sole hub. Camera scanning accepts only 1D Code 128 tracking barcodes and ignores QR codes. Captured or manually supplied values are decoded locally first; online lookup then supplies authoritative parcel details and status, while offline views are explicitly read-only cached projections.
