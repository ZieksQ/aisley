...
- Hub selector fix (2026-09-19): list every other active Logistics hub by safe ID/name, even without a preconfigured outgoing connection. Saving a hub mapping atomically creates or enables only the authenticated hub's outgoing connection, without Admin permission; reject self, unavailable targets, stale plans, and foreign lanes. Show a Beta tag on the Sort plan sidebar entry. Linehaul groups eligible sorted parcels automatically from the configured next-hub mapping; operators confirm only physical departure and do not enter parcel references.
