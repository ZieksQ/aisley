...
- Workspace redesign (2026-09-20): plan creation and editing now use one viewport-bounded dialog. At desktop widths, plan information, mappings ordered by physical lane number, physical lanes, and supported postal codes occupy the left side; selectable plans and their Save/Activate/Delete actions occupy the right side. Narrow screens put the selector first. Linehaul partner requests have their own dialog; grouped departure and receipt controls now appear in Sorting. The newer Linehaul contract in `docs/features/logistics/hub-to-hub-routing/specs.md` governs the still-pending approval and postal enforcement changes.
