...
- Keep Seller/Customer/Courier projections isolated from Logistics lane UI.
...
- Linehaul is a separate manifest flow for parcels whose committed route still requires another hub. Group only by the immediate next hub; atomic departure and independently verified parcel receipts never create final-mile tasks. Manifest members cannot use individual transfer endpoints. Final-mile schedule rules below remain destination-only. Company-truck receipts may sort/dispatch while unloading continues, but unreceived parcels and damaged condition holds cannot sort or dispatch. Release requires receiving Logistics inspection with a reason. See the current Linehaul contract revision in `docs/features/logistics/hub-to-hub-routing/specs.md`.
