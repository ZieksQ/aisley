...
- **Published-policy Edit:** Admin Edit must create the next numbered Draft copied from the selected published version (title, content, re-consent setting, creator, and optional source-version lineage), never update that published row or its acceptances/audit history. Only Drafts accept ordinary updates; publishing the successor atomically replaces the current version. Return the existing successor Draft or `409` rather than create competing copies, and audit successor creation separately.
- **User views:** the ordinary page/API returns only the current published version. A separate history view lists and renders exact published/superseded versions only, with version/title/effective date/current state and optional safe change summary; it never exposes Drafts, Admin metadata, or Internal Rules outside their audience.
### Policy publishing
...
- User-facing applications need a safe way to retrieve:
  ...
  - a policy's published/superseded version history when the user requests it
...
- [ ] Admin selecting Edit on a published policy receives a copied successor Draft; the published version remains byte-for-byte unchanged.
- [ ] Saving or abandoning a successor Draft does not change the latest user-facing policy.
...
- [ ] The normal user policy view returns only the latest effective version.
- [ ] A user-authorized history view lists only published/superseded versions and can render an exact historical version.
...
- Use `source_policy_version_id` only as version lineage; it must not make historical content mutable.
...
POST /api/admin/platform-settings/policy-versions/{version}/successor
PATCH /api/admin/platform-settings/policy-versions/{version}
...
GET  /api/policies/{type}/versions/{version}
...
### Published-policy edit transaction
- `POST .../policy-versions/{version}/successor` must lock the selected version and its policy identity.
- Reject non-published selections and stale requests with `409`.
- Create the next numbered Draft from the selected published content in the same transaction; do not update the selected row.
- Record the successor-creation audit event before commit.
- Return the new Draft DTO for the Admin editor. The browser must edit that Draft ID, not the published ID.
...
- Build:
  ...
  - policy type list
  - latest-policy summary with an Edit action that creates a successor Draft
  - policy version history, separate from the ordinary user policy view
...
### Tests
- **Laravel:** guest/non-Admin/permission denial; announcement create/update/publish/archive; draft visibility and expiration; safe content serialization; successor-Draft creation from a published policy; immutable published source; no user-facing change before successor publication; one-current-version invariant; concurrent successor/publish conflict; re-consent persistence; exact-version acceptance; idempotent acceptance; current-only and historical user DTO privacy; cache invalidation; audit creation; after-commit notification behavior.
- **Frontend:** announcement list/editor states; publish/archive confirmation; published-policy Edit creates and opens a successor Draft; policy history/current-view separation; history-version rendering; editor validation; re-consent selection; conflict refresh; forbidden state; safe preview rendering; accessibility.
...
- **Misleading edit UX:** labeling an in-place update as Edit can hide that it creates a successor Draft; the UI must state this before editing.
