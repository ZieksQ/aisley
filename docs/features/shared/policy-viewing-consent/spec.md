...
- Development bootstrap data lives in `src/api/database/seeders/data/platform-policies.json` and is loaded by `PlatformPolicySeeder`. It creates one published placeholder version per allow-listed policy only when that policy has no existing versions; rerunning the seeder never overwrites published content or acceptance history, and the placeholder seeder is skipped in production.
