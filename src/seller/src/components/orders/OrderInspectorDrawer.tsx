      ...
      <div className="space-y-6 text-xs">
          ...
          <div className="p-4 rounded-2xl bg-sky-50 dark:bg-sky-950/60 border border-sky-200 dark:border-sky-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-sky-900 dark:text-sky-200 flex items-center gap-1.5">
                <FaTruckFast className="text-sky-600 dark:text-sky-400" /> Logistics Partner: {order.courier.name}
              </span>
              <span className="font-mono-num font-bold text-sky-800 dark:text-sky-300 text-[11px]">
            ...
            {order.courier.courierNotes && (
              <p className="text-slate-600 dark:text-slate-400 text-[11px] italic">Notes: {order.courier.courierNotes}</p>
            )}
          </div>
        )}

        {/* Itemized Purchases Table */}
        <div className="rounded-2xl border border-slate-300 dark:border-slate-800 p-4 space-y-3 bg-white dark:bg-[#0F172A] shadow-sm">
          <h3 className="font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
            ...
            {order.items.map((item) => (
              <div key={item.id} className="py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-slate-900 dark:text-white truncate">{item.productTitle}</p>
                  <p className="text-slate-500 dark:text-slate-400 text-[11px]">{item.variantName || 'Standard Variant'}</p>
                  <p className="text-slate-400 dark:text-slate-500 font-mono-num text-[10px]">SKU: {item.sku}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="font-black text-slate-900 dark:text-white font-mono-num">{formatPHP(item.unitPrice)}</p>
                  <p className="text-slate-500 dark:text-slate-400 font-mono-num text-[11px]">Qty: {item.quantity}</p>
                  <p className="text-slate-400 dark:text-slate-500 font-mono-num text-[10px]">
          ...
          {order.customerNotes && (
            <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/60 border border-amber-300 dark:border-amber-700 text-amber-900 dark:text-amber-300 text-[11px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 rounded-2xl bg-[#F8FAFC] dark:bg-slate-900/60 border border-slate-300 dark:border-slate-800 space-y-1.5">
            <h4 className="font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
            ...
            <p className="text-slate-500 dark:text-slate-400 flex items-center gap-1 font-mono-num">
          ...
          <div className="p-4 rounded-2xl bg-[#F8FAFC] dark:bg-slate-900/60 border border-slate-300 dark:border-slate-800 space-y-1.5">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-mono-num text-[11px]">Tel: {order.shippingAddress.phone}</p>
          </div>
        </div>

        {/* ERP Financial Net Payout Breakdown */}
        <div className="rounded-2xl border-2 border-slate-900 dark:border-slate-700 bg-white dark:bg-[#0F172A] p-5 space-y-3 shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
            <h4 className="font-bold uppercase tracking-wider text-slate-900 dark:text-white flex items-center gap-1.5">
              <FaReceipt className="text-[#E723A2]" /> ERP Net Settlement Statement
            </h4>
            <span className="font-mono-num font-bold text-[11px] text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded-full border border-emerald-300 dark:border-emerald-800">
          ...
          <div className="space-y-1.5 text-xs">
            <div className="flex justify-between text-slate-600 dark:text-slate-300">
              ...
              <span className="font-bold font-mono-num text-slate-900 dark:text-white">{formatPHP(order.subtotal)}</span>
            </div>
            ...
            <div className="flex justify-between text-slate-500 dark:text-slate-400">
              ...
              <span className="font-bold font-mono-num text-rose-600 dark:text-rose-400">-{formatPHP(order.platformFee)}</span>
            </div>

            {order.shippingSubsidy > 0 && (
              <div className="flex justify-between text-slate-500 dark:text-slate-400">
                <span>Shipping Subsidy</span>
                <span className="font-bold font-mono-num text-rose-600 dark:text-rose-400">-{formatPHP(order.shippingSubsidy)}</span>
              </div>
            )}

            <div className="border-t-2 border-slate-900 dark:border-slate-700 pt-3 flex justify-between items-baseline">
              <div>
                <span className="font-black text-sm text-slate-900 dark:text-white uppercase">Net Seller Disbursed Payout</span>
                <p className="text-[10px] text-slate-500 dark:text-slate-400">Credited to GCash / Bank account upon delivery</p>
              </div>
              <span className="text-xl font-black text-emerald-700 dark:text-emerald-400 font-mono-num">
        ...
        {order.status !== 'cancelled' && order.status !== 'delivered' && (
          <div className="pt-2">
            <button
              ...
              className="w-full py-2.5 rounded-xl border border-rose-200 dark:border-rose-900/60 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer"
            >
