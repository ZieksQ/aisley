    ...
    {!order && loading ? <p className="mt-5" role="status">Loading order…</p> : order ? <>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
        ...
        <div className="space-y-5">
          ...
          <section className={`${orderPanel} p-5`}><h3 className="font-semibold">Waybill</h3>{order.waybill ? <><p className="mt-2 break-all text-sm font-medium">Tracking ID: {order.waybill.tracking_id ?? order.waybill.reference}</p><p className="mt-1 text-xs text-zinc-500">Thin Code 128 barcode · Created {orderDate(order.waybill.created_at)}</p><Link className={`${orderLink} mt-3 inline-block text-sm`} to="/orders/pickup">Preview, download, or print</Link></> : <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">The waybill is created when you select a Logistics provider and request pickup.</p>}</section>
        </div>
      </div>
