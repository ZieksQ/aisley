...
import { FaBoxOpen, FaPen, FaPlus, FaRotateRight } from 'react-icons/fa6'
...
export function ProductsPage() {
  ...
  async function transition(product: Product, action: 'publish' | 'archive') {
    ...
    try {
      ...
      setError(caughtError instanceof ApiError ? caughtError.message : `Unable to ${action} this product.`)
    }
  }
  ...
  async function confirmUnarchive() {
    ...
    setError('')
    try {
      await apiRequest(`/api/v1/seller/products/${productToUnarchive.id}/unarchive`, { method: 'POST' })
      setProductToUnarchive(null)
      setReload((value) => value + 1)
    } catch (caughtError) {
      setError(caughtError instanceof ApiError ? caughtError.message : 'Unable to restore this product.')
    } finally {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <div className="mt-5 overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-white/10 dark:bg-[#18181b]">
      {!page ? <p className="p-5 text-sm text-zinc-500">Loading products…</p> : page.data.length === 0 ? <div className="p-8 text-center"><FaBoxOpen className="mx-auto text-zinc-400" /><p className="mt-3 text-sm text-zinc-500">No products match this view.</p></div> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-zinc-200 bg-zinc-50 text-xs uppercase text-zinc-500 dark:border-white/10 dark:bg-white/[0.03]"><tr><th className="px-4 py-3">Product</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Price</th><th className="px-4 py-3">Available</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-zinc-200 dark:divide-white/10">{page.data.map((product) => <tr key={product.id}><td className="px-4 py-4"><Link className="font-medium text-[#4C1268] dark:text-purple-300" to={`/products/${product.id}/edit`}>{product.name}</Link><p className="mt-1 text-xs text-zinc-500">{product.category ?? 'No category'}</p></td><td className="px-4 py-4 capitalize">{product.status}</td><td className="px-4 py-4 tabular-nums">₱{Number(product.price).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td><td className="px-4 py-4 tabular-nums">{product.skus.reduce((n, sku) => n + sku.available, 0)}</td><td className="px-4 py-4"><div className="flex justify-end gap-4">{product.status === 'draft' ? <><Link className="inline-flex items-center gap-1.5 font-medium text-zinc-700 hover:text-zinc-950 dark:text-zinc-300 dark:hover:text-white" to={`/products/${product.id}/edit`}><FaPen aria-hidden="true" />Edit</Link><button className="font-medium text-[#4C1268] dark:text-purple-300" onClick={() => void transition(product, 'publish')}>Publish</button></> : product.status === 'active' ? <button className="font-medium text-zinc-600 hover:text-zinc-950 dark:text-zinc-300 dark:hover:text-white" onClick={() => void transition(product, 'archive')}>Archive</button> : <button className="font-medium text-[#4C1268] dark:text-purple-300" onClick={() => setProductToUnarchive(product)}>Unarchive</button>}</div></td></tr>)}</tbody></table></div>}
    </div>
    {productToUnarchive ? <div aria-labelledby="unarchive-title" aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-black/55 p-4" role="dialog"><div className="w-full max-w-md rounded-lg border border-zinc-200 bg-white p-5 shadow-lg dark:border-white/10 dark:bg-[#18181b]"><h3 className="text-lg font-semibold" id="unarchive-title">Unarchive product?</h3><p className="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-300"><strong>{productToUnarchive.name}</strong> will return to Draft. Its inventory will be active again, but buyers will not see it until you publish it.</p><div className="mt-5 flex justify-end gap-3"><button className="h-10 rounded-lg border border-zinc-300 px-4 text-sm font-medium dark:border-white/15" disabled={isRestoring} onClick={() => setProductToUnarchive(null)}>Cancel</button><button className="h-10 rounded-lg bg-[#4C1268] px-4 text-sm font-medium text-white disabled:opacity-50" disabled={isRestoring} onClick={() => void confirmUnarchive()}>{isRestoring ? 'Restoring…' : 'Unarchive'}</button></div></div></div> : null}
  </div>
}
