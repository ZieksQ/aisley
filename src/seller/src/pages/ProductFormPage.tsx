...
const empty = { name: '', category_id: '', sku: '', short_description: '', description_markdown: '', price: '', original_price: '', opening_stock: '0', shipping_weight_grams: '', shipping_length_mm: '', shipping_width_mm: '', shipping_height_mm: '', unit_cost: '' }
...
export function ProductFormPage() {
  ...
  useEffect(() => {
    ...
    apiRequest<{ data: Product }>('/api/v1/seller/products/' + productId).then(({ data }) => {
      setForm({ ...empty, name: data.name, category_id: data.category_id, short_description: data.short_description ?? '', description_markdown: data.description_markdown ?? '', price: data.price, original_price: data.original_price ?? '', sku: data.base_sku, opening_stock: String(data.skus[0]?.on_hand ?? 0), shipping_weight_grams: String(data.shipping_weight_grams ?? ''), shipping_length_mm: String(data.shipping_length_mm ?? ''), shipping_width_mm: String(data.shipping_width_mm ?? ''), shipping_height_mm: String(data.shipping_height_mm ?? ''), unit_cost: data.unit_cost_cents === null ? '' : (data.unit_cost_cents / 100).toFixed(2) })
      ...
      setVariants(data.variants.map((variant) => ({
        key: crypto.randomUUID(), id: variant.id,
  ...
  return <div className="mx-auto max-w-5xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <form className="mt-6 space-y-6" onSubmit={submit}>
      ...
      <section className="space-y-4 rounded-lg border border-zinc-200 bg-white p-5 dark:border-white/10 dark:bg-[#18181b]" aria-labelledby="variants-heading"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-semibold" id="variants-heading">Variants</h3><p className="mt-1 text-sm text-zinc-500">Add up to three options, then add each sellable combination manually. Adding a value keeps existing variants; choose it in a new row when needed. Blank prices inherit the Product price.</p></div><button className="inline-flex h-9 items-center gap-2 rounded-lg border border-zinc-300 px-3 text-sm font-medium dark:border-white/15" onClick={addOptionGroup} disabled={groups.length >= 3} type="button"><FaPlus /> Add option</button></div>
