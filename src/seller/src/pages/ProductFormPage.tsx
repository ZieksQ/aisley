...
type Variant = { key: string; option_value_indexes: number[]; labels: string[]; sku: string; price: string; original_price: string; status: 'active' | 'inactive'; image_upload_id?: string; image_preview?: string; available?: number }
...
export function ProductFormPage() {
  ...
  useEffect(() => {
    ...
    apiRequest<{ data: Product }>(`/api/v1/seller/products/${productId}`).then(({ data }) => {
      ...
      setVariants(data.variants.map((variant) => ({
        ...
        sku: variant.sku, price: variant.price ?? '', original_price: variant.original_price ?? '', status: variant.status, available: variant.available,
      })))
      setLoading(false)
  ...
  function generateVariants() {
    ...
    setVariants((current) => next.map((row) => {
      ...
      return current.find((variant) => variant.key === key) ?? { key, option_value_indexes: row.indexes, labels: row.labels, sku: `${form.sku || 'SKU'}-${row.labels.map((label) => label.replace(/[^a-z0-9]+/gi, '').toUpperCase()).join('-')}`, price: '', original_price: '', status: 'active' }
    }))
  ...
  async function submit(event: FormEvent) {
    ...
    const matrix = variants.map(({ sku, price, original_price, status, option_value_indexes, image_upload_id }) => ({ sku, price: price || null, original_price: original_price || null, status, option_value_indexes, image_upload_id: image_upload_id || null }))
    ...
    try {
      const response = await apiRequest<{ data: Product }>(productId ? `/api/v1/seller/products/${productId}` : '/api/v1/seller/products', { method: productId ? 'PATCH' : 'POST', body: JSON.stringify(payload) })
      setDirty(false)
      navigate(`/products/${response.data.id}/edit`, { replace: true })
      if (productId) window.location.reload()
  ...
  return <div className="mx-auto max-w-5xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <form className="mt-6 space-y-6" onSubmit={submit}>
      ...
      <section className="space-y-5 rounded-lg border border-zinc-200 bg-white p-5 dark:border-white/10 dark:bg-[#18181b]" aria-labelledby="details-heading"><h3 className="text-lg font-semibold" id="details-heading">Product details</h3>
        ...
        <div className="grid gap-5 sm:grid-cols-2">{field('sku', 'Base SKU', true)}{field('opening_stock', 'Opening stock', true, 'number')}</div>
      ...
      <section className="space-y-4 rounded-lg border border-zinc-200 bg-white p-5 dark:border-white/10 dark:bg-[#18181b]" aria-labelledby="variants-heading"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-semibold" id="variants-heading">Variants</h3><p className="mt-1 text-sm text-zinc-500">Add up to three options. Blank variant prices inherit the Product price.</p></div><button className="inline-flex h-9 items-center gap-2 rounded-lg border border-zinc-300 px-3 text-sm font-medium dark:border-white/15" onClick={() => { setGroups((current) => [...current, { name: '', values: [''] }]); setVariantsDirty(true); setDirty(true) }} disabled={groups.length >= 3} type="button"><FaPlus /> Add option</button></div>
        ...
        {groups.length ? <button className="h-10 rounded-lg bg-zinc-900 px-4 text-sm font-medium text-white dark:bg-zinc-100 dark:text-zinc-900" onClick={generateVariants} type="button">Generate {matrixSize || ''} combinations</button> : null}
        {variants.length ? <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-zinc-200 text-xs text-zinc-500 dark:border-white/10"><tr><th className="py-2 pr-3">Combination</th><th className="px-3 py-2">SKU</th><th className="px-3 py-2">Price override</th><th className="px-3 py-2">Original override</th><th className="px-3 py-2">Image</th><th className="pl-3 py-2">Stock</th></tr></thead><tbody className="divide-y divide-zinc-200 dark:divide-white/10">{variants.map((variant, index) => <tr key={variant.key}><td className="py-3 pr-3 font-medium">{variant.labels.join(' / ')}</td><td className="px-3 py-3"><input aria-label={`${variant.labels.join(' / ')} SKU`} className="h-9 w-40 rounded-lg border border-zinc-300 px-2 dark:border-white/15 dark:bg-[#18181b]" onChange={(event) => { setVariants((current) => current.map((item, position) => position === index ? { ...item, sku: event.target.value } : item)); setVariantsDirty(true); setDirty(true) }} required value={variant.sku} /></td><td className="px-3 py-3"><input aria-label={`${variant.labels.join(' / ')} price override`} className="h-9 w-32 rounded-lg border border-zinc-300 px-2 dark:border-white/15 dark:bg-[#18181b]" min="0.01" onChange={(event) => { setVariants((current) => current.map((item, position) => position === index ? { ...item, price: event.target.value } : item)); setVariantsDirty(true); setDirty(true) }} placeholder={`Inherit ${form.price || 'base'}`} step="0.01" type="number" value={variant.price} /></td><td className="px-3 py-3"><input aria-label={`${variant.labels.join(' / ')} original price override`} className="h-9 w-32 rounded-lg border border-zinc-300 px-2 dark:border-white/15 dark:bg-[#18181b]" min="0.01" onChange={(event) => { setVariants((current) => current.map((item, position) => position === index ? { ...item, original_price: event.target.value } : item)); setVariantsDirty(true); setDirty(true) }} placeholder="Inherit" step="0.01" type="number" value={variant.original_price} /></td><td className="px-3 py-3"><label className="cursor-pointer text-sm font-medium text-[#4C1268] dark:text-purple-300">{variant.image_preview ? 'Replace' : 'Add image'}<input accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) { setVariantsDirty(true); void uploadImage(file, 'variant', index) } }} type="file" /></label></td><td className="pl-3 py-3 tabular-nums">{variant.available ?? 0} <span className="text-xs text-zinc-500">via Inventory</span></td></tr>)}</tbody></table></div> : null}
      </section>
