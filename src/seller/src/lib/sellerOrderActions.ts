...
export function createPickupRequest(sellerId: string, orderIds: string[], logisticsOrganizationId: string, pickupAddressId: string) {
  const identity = [...orderIds].sort().join(':')
  return createIdempotentAction<{ data: { id: string; status: string; pickup_date: string | null; pickup_address_id: string; logistics_organization_id: string; order_ids: string[]; waybills: Array<{ id: string; order_id: string; reference: string; tracking_id: string; created_at: string; printable: boolean }> } }>(`seller-pickup:${sellerId}:${identity}:${logisticsOrganizationId}:${pickupAddressId}`, '/api/v1/seller/orders/pickup-requests', { order_ids: orderIds, logistics_organization_id: logisticsOrganizationId, pickup_address_id: pickupAddressId })
}
