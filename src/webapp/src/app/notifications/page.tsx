...
export default async function NotificationsPage() { const [auth, homepage] = await Promise.all([getServerAuthState(), getPublicHomepage(marketplaceConfig.discoveryPageSize)]); if (auth.status === "guest") redirect(`/login?next=${encodeURIComponent("/notifications")}`); return <HomeDataProvider initialData={homepage} trackView={false}><UtilityBar /><MarketplaceHeader /><main className="mx-auto w-full max-w-[900px] flex-1 px-4 py-7 sm:px-5 lg:px-8"><h1 className="mb-5 text-2xl font-semibold text-[#281E2C]">Notifications</h1><NotificationsPageContent /></main></HomeDataProvider>; }
