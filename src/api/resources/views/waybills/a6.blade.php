...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .barcode { display: block; width: 78mm; height: 9mm; margin: 1.2mm auto 0; }
...
@foreach ($labels as $label)
    ...
    <table class="label{{ $loop->last ? ' label-last' : '' }}">
        ...
        <tbody>
            ...
            <tr><td class="reference-cell" colspan="2"><div class="title">Tracking ID</div><div class="ref">{{ $s['tracking_id'] ?? $s['waybill_reference'] }}</div><img class="barcode" src="{{ $label['barcode'] }}" alt="Code 128 barcode for tracking ID {{ $s['tracking_id'] ?? $s['waybill_reference'] }}"><div class="order">CODE 128 · ORDER {{ $s['order_reference'] }} · CREATED {{ $s['created_at'] }}</div></td></tr>
