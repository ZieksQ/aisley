...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .barcode-frame { width: 90mm; padding: 0 3mm; margin: 1.2mm auto 0; background: #fff; }
        .barcode { display: block; width: 84mm; height: 12mm; }
...
@foreach ($labels as $label)
    ...
    <table class="label{{ $loop->last ? ' label-last' : '' }}">
        ...
        <tbody>
            ...
            <tr><td class="reference-cell" colspan="2"><div class="title">Tracking ID</div><div class="ref">{{ $s['tracking_id'] ?? $s['waybill_reference'] }}</div><div class="barcode-frame"><img class="barcode" src="{{ $label['barcode'] }}" alt="Code 128 barcode for tracking ID {{ $s['tracking_id'] ?? $s['waybill_reference'] }}"></div><div class="order">CODE 128 · ORDER {{ $s['order_reference'] }} · CREATED {{ $s['created_at'] }}</div></td></tr>
            ...
            <tr><td class="footer-cell"><span class="cod">COD</span><div class="amount">{{ $s['payment']['currency'] }} {{ number_format((float) $s['payment']['collectible_amount'], 2) }}</div><div class="quantity">Parcel item quantity<br><strong>{{ $s['item_quantity'] }}</strong></div><div class="small handling">Keep this label flat, dry, and fully visible.</div></td><td class="footer-cell qr"><img src="{{ $label['qr'] }}" alt="Waybill QR code"><div class="small">QR fallback · authorized Aisley app<br>{{ $s['tracking_id'] ?? $s['waybill_reference'] }}</div></td></tr>
        </tbody>
    </table>
@endforeach
</body>
