...
class CustomerOrderStatusTest extends TestCase
{
    ...
    public function test_owned_detail_returns_safe_snapshots_timeline_map_state_and_actions(): void
    {
        ...
        $response = $this->getJson('/api/v1/customer/orders/'.$order->id)
            ...
            ->assertJsonPath('data.timeline.1.label', 'Received at sorting hub')
            ->assertJsonPath('data.timeline.1.eventType', 'hub_received')
            ->assertJsonPath('data.timeline.1.location.hub', 'Makati Hub')
            ->assertJsonPath('data.timeline.1.occurredAt', $eventTime->toISOString())
