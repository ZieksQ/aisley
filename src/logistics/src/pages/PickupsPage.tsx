...
export function PickupsPage() {
  ...
  return <div className="mx-auto max-w-[1400px] px-4 py-5 sm:px-5 lg:px-6">
        ...
        <section className="min-h-0 border-b border-zinc-200 lg:flex lg:flex-col lg:border-b-0 lg:border-r dark:border-white/10">
          ...
          {pendingLoading && !pending ? <p className="p-5 text-sm" role="status">Loading pending parcels…</p> : pending?.data.length ? <ul aria-label="Pending pickup parcels" className="min-h-0 max-h-[calc(90vh-13rem)] flex-1 divide-y divide-zinc-200 overflow-y-auto overscroll-contain dark:divide-white/10">{pending.data.map((pickup) => {
            ...
            return <li key={pickup.id}>
              ...
              <ul className="divide-y divide-zinc-100 dark:divide-white/5">{available.map((order) => <li className="flex items-center gap-3 px-5 py-2.5 pl-12" key={order.id}><input aria-label={`Select parcel ${order.reference}`} checked={selectedIds.has(order.id)} className="size-4 accent-[#4C1268]" disabled={!selectedIds.has(order.id) && selected.length >= 30} onChange={() => toggleParcel(pickup, order)} type="checkbox" /><div className="min-w-0"><p className="text-sm font-medium">{order.reference}</p><p className="text-xs text-zinc-500">Tracking ID: {order.waybill?.tracking_id ?? order.waybill?.reference ?? 'unavailable'}</p></div></li>)}</ul>
            </li>
