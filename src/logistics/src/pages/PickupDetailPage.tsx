...
function ScheduleDialog({ dialog, busy, error, title, description, submitLabel, submit, ...fields }: React.ComponentProps<typeof ScheduleFields> & { dialog: React.RefObject<HTMLDialogElement | null>; busy: boolean; error: string; title: string; description: string; submitLabel: string; submit: () => void }) { return <dialog ref={dialog} className="m-auto w-[calc(100%-2rem)] max-w-lg rounded-lg border border-zinc-200 bg-white p-5 text-zinc-950 backdrop:bg-black/55 dark:border-white/15 dark:bg-[#18181b] dark:text-white"><h3 className="text-lg font-semibold">{title}</h3><p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">{description}</p><ScheduleFields {...fields} />{error ? <p className="mt-3 border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-400/30 dark:bg-red-400/10 dark:text-red-200" role="alert">{error}</p> : null}<p className="mt-3 text-xs text-zinc-500">Times are entered in Philippine Time (Asia/Manila) and stored as UTC.</p><div className="mt-5 flex justify-end gap-2"><ActionButton onClick={() => dialog.current?.close()}>Cancel</ActionButton><PrimaryButton busy={busy} onClick={submit}>{submitLabel}</PrimaryButton></div></dialog> }
