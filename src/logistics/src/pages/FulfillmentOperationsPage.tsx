...
import { ProofPhotoReview } from '../components/ProofPhotoReview'
...
function referenceFor(record: FulfillmentShipment): string {
  return record.parcel?.tracking_id ?? record.parcel?.waybill_reference ?? record.parcel?.order_reference ?? record.parcel?.reference ?? ''
}
...
export function FulfillmentOperationsPage() {
  ...
  const loadRecord = useCallback(async (value: string, decoded?: DecodedParcelBarcode) => {
    ...
    if (!nextReference) {
      setDetailError('This barcode does not contain a usable tracking ID, waybill, Order, Parcel, or shipment reference.')
      return
    }
  ...
  const handleScan = useCallback((raw: string) => {
    ...
    if (!decoded.reference) {
      setDetailError('Barcode decoded, but it does not contain a usable tracking ID or parcel reference.')
      return
    }
  ...
  useWaybillCamera(scannerOpen, videoRef, (raw) => { handleScan(raw) }, (message) => {
  ...
  function openRecord(item: FulfillmentShipment) {
    ...
    else setDetailError('This queue row does not have a usable tracking ID, waybill, or Order reference.')
  }
  ...
  return <div className="mx-auto max-w-[1500px] px-4 py-5 sm:px-5 lg:px-6">
    ...
    <form className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-[minmax(16rem,1fr)_15rem_15rem_auto]" onSubmit={applyFilters}>
      <label className="relative"><span className="sr-only">Search a tracking ID, waybill, Order, or Parcel reference</span><FaMagnifyingGlass className="pointer-events-none absolute left-3 top-3 text-zinc-400" aria-hidden="true" /><input className={`${field} pl-9`} onChange={(event) => setSearch(event.target.value)} placeholder="Tracking ID, waybill, Order, or Parcel" value={search} /></label>
    ...
    <div className="mt-4 grid items-start gap-4 lg:grid-cols-[minmax(19rem,24rem)_minmax(0,1fr)]">
      ...
      <section className={`${panel} min-h-[32rem]`} aria-label="Fulfillment record detail" aria-busy={detailLoading}>
        <div className="border-b border-zinc-200 px-4 py-3 dark:border-white/10"><h3 className="font-semibold">Tracking ID lookup</h3><form className="mt-3 flex flex-col gap-2 sm:flex-row" onSubmit={(event) => { event.preventDefault(); void loadRecord(reference) }}><label className="flex-1"><span className="sr-only">Tracking ID, waybill, Order, Parcel, or shipment reference</span><input className={field} onChange={(event) => setReference(event.target.value)} placeholder="Tracking ID, waybill, Order, Parcel, or shipment" value={reference} autoComplete="off" /></label><div className="flex gap-2"><button aria-label={scannerOpen ? 'Stop barcode scanner' : 'Start barcode scanner'} aria-pressed={scannerOpen} className={iconButton} onClick={() => setScannerOpen((value) => !value)} title={scannerOpen ? 'Stop barcode scanner' : 'Start barcode scanner'} type="button">{scannerOpen ? <FaStop aria-hidden="true" /> : <FaCamera aria-hidden="true" />}</button><PrimaryButton busy={detailLoading} type="submit">Look up</PrimaryButton></div></form>{scannerOpen ? <div className="mt-3 overflow-hidden border border-zinc-200 bg-black dark:border-white/10"><video ref={videoRef} className="aspect-video w-full object-contain" autoPlay muted playsInline /><p className="bg-zinc-950 px-3 py-2 text-xs text-zinc-300">Move close enough for the bars to fill most of the preview while keeping both ends and white margins visible. The waybill QR also works.</p></div> : null}{decodedParcel ? <DecodedParcelSummary decoded={decodedParcel} /> : null}</div>
        {detailError ? <div className="m-4"><ErrorNotice message={detailError} retry={() => void loadRecord(reference)} /></div> : null}
        {detailLoading && !record ? <p className="p-5 text-sm" role="status">Loading the authoritative record…</p> : record ? <><div className="px-4 pt-4 sm:px-5">{recordSource === 'offline-cache' ? <p className="border-l-2 border-amber-500 pl-3 text-xs leading-5 text-amber-800 dark:border-white/20 dark:text-amber-300" role="status">Cached projection only. This record is read-only until the current server state is loaded.</p> : null}</div><RecordDetail actionsEnabled={actionsEnabled} record={record} reason={reason} setReason={setReason} evidenceChoice={evidenceChoice} setEvidenceChoice={setEvidenceChoice} transitionBusy={transitionBusy} onTransition={(target, evidenceId) => void transition(target, evidenceId)} onProofRejected={() => { setEvidenceChoice({}); void loadRecord(reference) }} finalTask={finalTask} /></> : <div className="grid min-h-[24rem] place-items-center p-6 text-center text-sm text-zinc-500"><div><FaClipboardCheck className="mx-auto mb-3 text-2xl text-zinc-400" aria-hidden="true" /><p>Choose a queue row or enter a reference to inspect a shipment.</p><p className="mt-1 text-xs">Transitions and evidence decisions always come from the server.</p></div></div>}
      </section>
    </div>
  </div>
}
...
function RecordDetail({ actionsEnabled, record, reason, setReason, evidenceChoice, setEvidenceChoice, transitionBusy, onTransition, onProofRejected, finalTask }: RecordDetailProps) {
  const [reviewedProofId, setReviewedProofId] = useState<string | null>(null)
  ...
  function transitionControl(target: string) {
    ...
    return <div className="border-t border-zinc-200 pt-3 dark:border-white/10" key={target}><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-medium">{transitionLabels[target] ?? human(target)}</p><p className="mt-1 text-xs text-zinc-500">Current shipment state: {human(record.status)}.</p></div><PrimaryButton busy={transitionBusy} disabled={!actionsEnabled || ((requiresHubEvidence || requiresProof) && !selectedEvidence) || intentMissing || (requiresProof && reviewedProofId !== selectedProofEvidence)} onClick={() => onTransition(target, selectedEvidence || undefined)}>{transitionLabels[target] ?? human(target)}</PrimaryButton></div>{requiresHubEvidence ? <EvidenceSelector disabled={!actionsEnabled} label="Hub-pickup evidence" evidence={hubEvidence} value={selectedHubEvidence} onChange={(value) => chooseEvidence('picked_up_from_hub', value)} empty="Waiting for Courier hub-pickup submission." /> : null}{requiresProof ? <><EvidenceSelector disabled={!actionsEnabled} label="Delivery photo POD" evidence={proofEvidence} value={selectedProofEvidence} onChange={(value) => chooseEvidence('delivered', value)} empty="Waiting for Courier photo POD." />{selectedProofEvidence && proofEvidence.find((item) => item.id === selectedProofEvidence)?.type === 'photo' ? <ProofPhotoReview actionsEnabled={actionsEnabled} onRejected={onProofRejected} onViewed={setReviewedProofId} proofId={selectedProofEvidence} taskRevision={finalTask?.revision ?? 0} /> : null}{selectedProofEvidence && reviewedProofId !== selectedProofEvidence ? <p className="mt-2 text-xs text-zinc-500">Review the photo before confirming delivery.</p> : null}{intentMissing ? <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">A Courier completion intent for this proof is required before Logistics can validate delivery.</p> : selectedIntent ? <p className="mt-2 text-xs text-zinc-500">Completion intent: <StateLabel value={selectedIntent.status} /> · confirmed {formatDate(selectedIntent.confirmed_at)}</p> : null}</> : null}</div>
  }
  ...
  return <div className="p-4 sm:p-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs text-zinc-500">Shipment {record.shipment_id}</p><h4 className="mt-1 text-lg font-semibold">{record.parcel?.order_reference ?? record.parcel?.reference ?? 'Fulfillment record'}</h4><p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Tracking ID {record.parcel?.tracking_id ?? record.parcel?.waybill_reference ?? 'unavailable'} · {record.parcel?.item_count ?? 0} item{record.parcel?.item_count === 1 ? '' : 's'}</p></div><StateLabel value={record.status} /></div>
    {record.parcel?.order_id && !['delivered', 'in_transfer'].includes(record.status) ? <Link className="mt-3 inline-flex text-sm font-semibold text-[#4C1268] hover:underline dark:text-purple-300" to={`/messages?order_id=${record.parcel.order_id}`}>Message Customer</Link> : null}
    <dl className="mt-5 grid gap-3 border-y border-zinc-200 py-4 text-sm dark:border-white/10 sm:grid-cols-3"><div><dt className="text-zinc-500">Revision</dt><dd className="mt-1 font-mono">{record.revision}</dd></div><div><dt>Last activity</dt><dd className="mt-1">{formatDate(record.last_activity_at)}</dd></div><div><dt className="text-zinc-500">Allowed next states</dt><dd className="mt-1">{record.allowed_transitions.length || 'None'}</dd></div></dl>
...
function TaskSummary({ task }: { task: FulfillmentTask }) {
  const active = task.courier_id && (task.leg === 'first_mile'
    ? ['seller_pickup_assigned', 'seller_pickup_accepted'].includes(task.status)
    : ['delivery_assigned', 'delivery_accepted', 'picked_up_from_hub', 'in_transit', 'out_for_delivery'].includes(task.status))
  return <article className="border border-zinc-200 p-3 dark:border-white/10"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-medium uppercase tracking-wide text-zinc-500">{human(task.leg)}</p><p className="mt-1 text-sm font-medium">Task {task.task_id.slice(0, 8)} · revision {task.revision}</p></div><StateLabel value={task.status} /></div><dl className="mt-3 grid gap-2 text-xs text-zinc-500 sm:grid-cols-3"><div><dt>Courier</dt><dd className="mt-1 text-zinc-800 dark:text-zinc-200">{task.courier?.name ?? task.courier_id ?? 'Not assigned'}</dd></div><div><dt>Accepted</dt><dd className="mt-1">{formatDate(task.accepted_at)}</dd></div><div><dt>Picked up</dt><dd className="mt-1">{formatDate(task.picked_up_at)}</dd></div></dl>{active ? <Link className="mt-3 inline-flex text-sm font-semibold text-[#4C1268] hover:underline dark:text-purple-300" to={`/messages?leg=${task.leg}&task_id=${task.task_id}`}>Message Courier</Link> : null}{task.leg === 'final_mile' && task.failed_attempt_count ? <p className="mt-3 text-xs text-amber-800 dark:text-amber-300">{task.failed_attempt_count} failed delivery attempt{task.failed_attempt_count === 1 ? '' : 's'} · latest {human(task.failed_attempts?.[0]?.reason ?? 'unknown')} {formatDate(task.failed_attempts?.[0]?.attempted_at ?? null)}. Courier may retry.</p> : null}</article>
}
