  ...
  return <div className="mx-auto max-w-[1500px] px-4 py-5 sm:px-5 lg:px-6">
    ...
    <div className="mt-4 grid items-start gap-4 lg:grid-cols-[minmax(19rem,24rem)_minmax(0,1fr)]">
      ...
      <section className={`${panel} min-h-[32rem]`} aria-label="Fulfillment record detail" aria-busy={detailLoading}>
        <div className="border-b border-zinc-200 px-4 py-3 dark:border-white/10"><h3 className="font-semibold">Tracking ID lookup</h3><form className="mt-3 flex flex-col gap-2 sm:flex-row" onSubmit={(event) => { event.preventDefault(); void loadRecord(reference) }}><label className="flex-1"><span className="sr-only">Tracking ID, waybill, Order, Parcel, or shipment reference</span><input className={field} onChange={(event) => setReference(event.target.value)} placeholder="Tracking ID, waybill, Order, Parcel, or shipment" value={reference} autoComplete="off" /></label><div className="flex gap-2"><button aria-label={scannerOpen ? 'Stop barcode scanner' : 'Start barcode scanner'} aria-pressed={scannerOpen} className={iconButton} onClick={() => setScannerOpen((value) => !value)} title={scannerOpen ? 'Stop barcode scanner' : 'Start barcode scanner'} type="button">{scannerOpen ? <FaStop aria-hidden="true" /> : <FaCamera aria-hidden="true" />}</button><PrimaryButton busy={detailLoading} type="submit">Look up</PrimaryButton></div></form>{scannerOpen ? <div className="mt-3 overflow-hidden border border-zinc-200 bg-black dark:border-white/10"><video ref={videoRef} className="aspect-video w-full object-contain" autoPlay muted playsInline /><p className="bg-zinc-950 px-3 py-2 text-xs text-zinc-300">Scan the thin 1D Code 128 tracking barcode. Keep all bars and both white margins visible. QR codes are ignored.</p></div> : null}{decodedParcel ? <DecodedParcelSummary decoded={decodedParcel} /> : null}</div>
