...
function RecordDetail({ record, reason, setReason, evidenceChoice, setEvidenceChoice, transitionBusy, onTransition, finalTask }: RecordDetailProps) {
  ...
  return <div className="p-4 sm:p-5">
    ...
    <section className="mt-5"><div className="flex items-center gap-2"><FaRoute className="text-[#4C1268] dark:text-purple-300" aria-hidden="true" /><h5 className="font-semibold">Custody controls</h5></div><p className="mt-1 text-xs leading-5 text-zinc-500">Receiving, sorting, and dispatch use their dedicated workspaces. This page retains evidence validation and delivery recovery.</p>{record.status === 'received_at_hub' ? <Link className="mt-3 inline-flex text-sm font-semibold text-[#4C1268] hover:underline dark:text-purple-300" to="/sorting">Open Sorting</Link> : null}<label className="mt-3 block text-sm font-medium" htmlFor="transition-reason">Operational reason (optional)<textarea className={`${field} mt-1 h-20 py-2`} id="transition-reason" maxLength={1000} onChange={(event) => setReason(event.target.value)} placeholder="Add context for a manual recovery or handoff review" value={reason} /></label><div className="mt-4 space-y-3">{hubTransitions.length ? hubTransitions.map(transitionControl) : <p className="border-l-2 border-zinc-300 pl-3 text-sm text-zinc-500 dark:border-white/20">No Hub operations action is available from this state.</p>}</div></section>
